...
def ctg_create(request):
    ...
    if form.is_valid():
        ...
        instance.save()
        messages.success(request, instance.name + " Successfully Added ")
        return HttpResponseRedirect( "/blog/create/")
    context = {
        "form": form,
        "form_name": " Category Form "
    }
    return render(request, "ctg_form.html", context)

def subctg_create(request):
    if not (request.user.is_staff or request.user.is_superuser):
        raise Http404
...
def post_detail(request, slug, year, month):
    ...
    if form.is_valid() and request.user.is_authenticated():
        ...
        #    request.user)
        #sender = 'XChange Idea <Hello@xchangeidea.net>'
        #recipients = ['mail2raajj@gmail.com']
        #send_mail(subject, message, sender, recipients)

        subject = str(instance.user) +', Your Post has received a comment: ' + instance.title[0:20] + '.. '
        from_email = 'xchangeidea.net <hello@xchangeidea.net>'
        to = 'mail2raajj@gmail.com'

        #html_content = 'Hello '+ str(instance.user) +'<br/><p>Your post has received a message, below are the details:</p>' \
        #               '<strong>Post:</strong> '+ instance.title +'<br/><strong>Comment:</strong> '+str(content_data)+\
        #               '<br/><strong>Commented by User :</strong> '+str(request.user)+'<br/><br/> Thank you!!'
        ...
        html_content = get_template('blog/email_comment_message.html').render(Context(ctx))
